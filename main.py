    ...
    if(st.button("Show Image", disabled=(test_image is None))):
        if test_image is not None:
            try:
                ...
                st.error("An error occurred while displaying the image.")
    #Predict button
    if(st.button("Predict", disabled=(test_image is None))):
        if test_image is not None:
            ...
            try:
                ...
                logging.error("Error processing image prediction: %s", e)
                st.error("An error occurred while processing the image. Please try again with a valid image file.")
